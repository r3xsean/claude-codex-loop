---
name: codex
description: Second-model consultant via OpenAI Codex (GPT-5.4) for plans, designs, tradeoffs, debugging, and autonomous-loop escalations. Supports multi-turn resume. **AUTO-INVOKE without asking** at these moments — just fire, don't ask permission: (1) **DESIGN-PRESENTATION (primary) — BEFORE presenting any non-trivial plan, architecture, data model, approach comparison, or multi-step implementation to the user.** Fire on decision density (multiple coordinated steps OR non-obvious tradeoffs), not document formality. Fold Codex's feedback in — note agreements, surface disagreements plainly, show the delta if the plan changed. (2) **COMMITMENT (fallback)** — user commits to a non-obvious direction ("okay let's do X") without an explicit design presented first; auto-invoke BEFORE writing code. (3) **MID-LOOP UNBLOCKER** during "ship it" / "production ready" / "don't stop" loops — same tactic failed 2x, test fails inconsistently, stuck on approach, or /codex-review finding unclear if blocking vs nit. Consult Codex FIRST before escalating to user. SKIP trivial implementation, syntax lookups, things Claude knows cold.
when_to_use: Trigger phrases — "codex", "second opinion", "what does gpt think", "ask another model". Example moments to auto-fire — about to show the user a multi-step plan; user just said "okay let's do X" on a non-obvious choice; autonomous loop stuck after 2 same-tactic retries; about to give a strong recommendation on an approach.
