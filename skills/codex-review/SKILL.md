---
name: codex-review
description: Adversarial cross-model code review via OpenAI Codex. Hunts for AI-coding-agent failure modes — false completion claims, silent assumptions, race conditions, incomplete wiring, dead code, production failure modes. Not a linter. **Default behavior: auto-invoke at COMPLETION moments** — before reporting a finished non-trivial change to the user. Do not ask permission; just run it. TRIGGER when (1) user asks for "review", "check my changes", "did I miss anything", "look this over", "adversarial review"; (2) you just finished implementing a non-trivial feature or change and are about to report completion — AUTO-INVOKE, don't ask; (3) user is about to commit, ship, or push; (4) finished change touches concurrency/async, auth/security boundaries, migrations, state machines, caching, API contracts, or anything where "works locally" is a weak signal. Do NOT trigger for cosmetic edits, renames, test-only changes, formatting, dependency bumps, trivial bugfixes, OR mid-implementation (only at completion — if the user wants a mid-implementation review they'll ask explicitly). One review per meaningful unit of work, not per file-edit. Budget is not a constraint.
